...
class SolarSystem:
    def __init__(self,panel_wattage,number_of_panels,panel_length,panel_width,roof_length,roof_width,month,start_year,current_year):
        ...
        self.start_year=start_year                    # int number
        self.current_year=current_year                # int number
...
class EnergyGeneration:
    ...
    def calculate_daily_energy(self):
        return (self.ss_obj.panel_wattage            #object accession
                ...
                *self.ss_obj.number_of_panels).item()       #object accession
    
    def calculate_monthly_energy(self):
        days=DAYS_IN_A_MONTH[DAYS_IN_A_MONTH["month"]==self.ss_obj.month]["no_of_days"][0].item()
    ...
    def calculate_annual_energy_in_months(self,panel_year):      # new
        ...
        decay_factor=PANEL["efficiency_loss"][0]**(panel_year-1)
        return annual_peak_energy_in_months*decay_factor

    def calculate_annual_energy(self):
        if self.ss_obj.current_year%4==0:
    ...
    def calculate_lifetime_energy(self):
        lifeline=np.arange(25)+self.ss_obj.start_year
        years=np.arange(PANEL["life"][0])
        decay_arr=(PANEL["efficiency_loss"][0])**years
        return self.calculate_daily_energy()*decay_arr*np.where(lifeline%4==0,366,365)
