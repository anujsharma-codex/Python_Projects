...
class Location:
    ...
    def get_city_latitude(self):
        return (CITY[np.where(CITY["city_name"]==self.city)]["latitude"][0]).item()
    
    def get_city_longitude(self):
        return (CITY[np.where(CITY["city_name"]==self.city)]['longitude'][0]).item()
    
    def get_city_max_peak_sun_hours(self):
        return (CITY[np.where(CITY["city_name"]==self.city)]["peak_sun_hours"][0]).item()
    ...
    def get_combined_subsidy(self):
        ...
        return city_subsidy.item()
    
    def get_state_export_rate(self):
        return (STATE[np.where(STATE["state_name"]==self.state)]["export_rate"][0]).item()
    
    def get_state_tariff(self):
        return (STATE[np.where(STATE["state_name"]==self.state)]["tariff"][0]).item()